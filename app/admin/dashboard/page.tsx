...
const DASHBOARD_COPY = {
  hu: {
    ...
    calendar: {
      ...
      today: "ma",
      next7Days: "ebben a hónapban",
      attention: "nyitott teendő",
      daysTitle: "Havi nézet",
      quietDaysTitle: "Ezen a napon nincs esemény",
      quietDaysDescription: "Kattints egy jelölt napra a napi munkákhoz, teendőkhöz és aktivitásokhoz.",
      eventCount: (count: number) => `${count} esemény`,
      shownCount: (shown: number, total: number) => `${shown} mutatva / ${total} összes`,
      agendaTitle: "Következő események",
      agendaDescription: "A hónap legközelebbi munkái, teendői és ügyfélaktivitásai időrendben.",
  ...
  de: {
    ...
    calendar: {
      ...
      today: "heute",
      next7Days: "diesen Monat",
      attention: "offene Aufgaben",
      daysTitle: "Monatsansicht",
      quietDaysTitle: "An diesem Tag gibt es keine Termine",
      quietDaysDescription: "Klicke auf einen markierten Tag, um Arbeiten, Aufgaben und Aktivitäten zu sehen.",
      eventCount: (count: number) => `${count} Termine`,
      shownCount: (shown: number, total: number) => `${shown} gezeigt / ${total} gesamt`,
      agendaTitle: "Nächste Termine",
      agendaDescription: "Die nächsten Arbeiten, Aufgaben und Kundenaktivitäten dieses Monats in chronologischer Reihenfolge.",
      legendTitle: "Legende",
      legendTask: "Aufgabe",
      legendEvent: "Arbeit",
  ...
  en: {
    ...
    calendar: {
      ...
      quietDaysTitle: "No events on this day",
      quietDaysDescription: "Click a marked day to see its work, tasks and activity.",
      eventCount: (count: number) => `${count} events`,
      shownCount: (shown: number, total: number) => `${shown} shown / ${total} total`,
      agendaTitle: "Next events",
      agendaDescription: "The next work items, tasks and client activity from this month in chronological order.",
      legendTitle: "Legend",
      legendTask: "Task",
      legendEvent: "Work",
        ...
        <div className="rounded-md border border-ink/8 bg-white">
          <div className="flex flex-col justify-between gap-3 border-b border-ink/8 px-4 py-3 sm:flex-row sm:items-start">
            ...
            <span className="rounded-full bg-ink/[0.04] px-2.5 py-1 text-xs font-medium text-graphite/60">
              {copy.calendar.shownCount(orderedEvents.length, monthEvents.length)}
            </span>
          </div>
...
export default async function AdminDashboardPage() {
  ...
  const dashboardTasks = sortDashboardTasks([
    ...
    ...actionableZipPackages.map((downloadPackage): DashboardTask => ({
      ...
      href: `/admin/galleries/${downloadPackage.gallery.id}?tab=activity`,
      ...
      return [
        {
          ...
          href: `/admin/galleries/${list.gallery.id}?tab=activity`,
