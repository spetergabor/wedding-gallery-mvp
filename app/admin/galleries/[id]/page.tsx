...
export default async function GalleryDetailPage({
  ...
  const selectedGalleryTextColor = galleryTextColorOrDefault(
    ...
    selectedGalleryDesign === GALLERY_DESIGN_COVER_STICKY || selectedGalleryDesign === GALLERY_DESIGN_FULLSCREEN_COVER
      ? "#ffffff"
      : "#111111"
  );
  const selectedGalleryBodyTextColor = galleryBodyTextColorOrDefault(gallery.galleryBodyTextColor);
      ...
      <div className="space-y-6">
        ...
        <div data-gallery-tab-panel="appearance" hidden={activeTab !== "appearance"}>
          <div className="space-y-6">
            ...
            <form
              action={updateGalleryDesignAction.bind(null, gallery.id)}
              className="rounded-lg border border-ink/10 bg-white p-6 shadow-soft"
              data-gallery-design-form
            >
              <GalleryDesignLiveControls />
              ...
              <input
                ...
                value={selectedClassicGradientIntensity}
                disabled={selectedGalleryDesign === GALLERY_DESIGN_CLASSIC}
                ...
                <div className="rounded-md border border-ink/10 bg-white p-3 lg:col-span-2">
                  <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)] lg:items-start">
                    <div>
                      ...
                      <p className="mt-1 text-sm leading-6 text-graphite/70">
                        Külön állítható a nagy hero cím és a galéria többi szövegének betűtípusa.
                      </p>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <label className="block space-y-2">
                        <span className="text-xs font-semibold uppercase tracking-[0.14em] text-graphite/55">Hero cím</span>
                        <select
                      ...
                      <label className="block space-y-2">
                        <span className="text-xs font-semibold uppercase tracking-[0.14em] text-graphite/55">Galéria szövegek</span>
                        <select
                          name="galleryBodyFont"
                          defaultValue={selectedGalleryBodyFont.key}
                          className="h-11 w-full rounded-md border border-ink/15 bg-white px-3 text-sm text-ink outline-none transition focus:border-ink/50"
                        >
                          {GALLERY_TITLE_FONTS.map((font) => (
                            <option key={font.key} value={font.key}>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <p className="rounded-md bg-paper px-3 py-2 text-sm text-graphite/70">
                      Hero: {selectedGalleryTitleFont.description}
                    </p>
                    <p className="rounded-md bg-paper px-3 py-2 text-sm text-graphite/70">
                      Szövegek: {selectedGalleryBodyFont.description}
                    </p>
                  </div>
                </div>
